...
```SQL
1. HAVING 절
SUM(PRICE)는 집계 함수이므로, WHERE 절에서 사용할 수 없음. SQL에서 WHERE 절은 개별 행(row)을 기준으로 필터링을 하는데, SUM(PRICE)와 같은 집계 함수는 전체 그룹화된 결과에 대해 적용되므로 HAVING 절을 사용해야 함.

2. GROUP BY
SUM(PRICE)와 같은 집계 함수를 사용할 때는 그룹화할 기준을 명시해야 함. 이때 GROUP BY 절에 포함되지 않은 컬럼은 SELECT 절에서 직접 사용할 수 없음.
즉, GROUP BY를 할 때 USER_ID와 NICKNAME 2개를 모두 써야 함. 하나만 사용해도 기술적으로 쿼리가 실행될 수 있으나, 위에 명시된 SQL의 기본 원칙에 따라 USER_ID와 NICKNAME 두개를 모두 쓰는 것이 안전함.
```
